...
- (instancetype)initWithMode:(PSWebSocketMode)mode request:(NSURLRequest *)request {
    if((self = [super init])) {
        ...
        _socketId = [[NSUUID UUID] UUIDString];
    }
    return self;
}
...
- (void)closeWithCode:(NSInteger)code reason:(NSString *)reason {
    [self executeWork:^{
        ...
        if(self->_readyState >= PSWebSocketReadyStateClosing) {
            return;
        }
        
        BOOL connecting = (self->_readyState == PSWebSocketReadyStateConnecting);
        self->_readyState = PSWebSocketReadyStateClosing;
